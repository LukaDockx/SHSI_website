...
- Blank/whitespace `Attendance Status` cells are grouped by program/faculty in activity mode and by housing group in housing mode, including rows whose Jumbula `Status` is `Checked out` or `Not checked in`. If an optional RA contacts CSV is uploaded, its contact info is added; if omitted, the report still runs without RA contact info. Checked groups generate sheets only for eligible active students; unchecked groups are treated as all present for sheet generation.
