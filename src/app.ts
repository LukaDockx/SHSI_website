...
(() => {
  ...
  function toggleMode() {
    ...
    try { localStorage.setItem(MODE_KEY, state.mode); } catch (_error) { /* ignore */ }
  ...
  function updateModeUi() {
    ...
    if (button) button.textContent = isHousingMode ? "Switch to activity attendance mode" : "Switch to housing attendance mode";
    ...
    if (runButton) runButton.textContent = isHousingMode ? "Review housing submissions" : "Review faculty submissions";

    const yesterdayZone = document.querySelector('[data-file-key="yesterday"]');
    if (yesterdayZone) yesterdayZone.hidden = isHousingMode;
    ...
    if (contactsZone) {
      ...
      if (help) help.textContent = isHousingMode ? "Optional housing/RA contact mapping, contact person, phone" : "Program, faculty, contact person, phone";
    }
  }
  ...
  function loadStoredFiles(mode = state.mode) {
    try {
      ...
      if (modeSpecific) return JSON.parse(modeSpecific) || {};
      ...
      if (mode !== "housing") return JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}") || {};
      return {};
    } catch (_error) {
      return {};
    }
  ...
  function isFileRequired(key) {
    if (key === "yesterday" && state.mode === "housing") return false;
    if (key === "faculty" && state.mode === "housing") return false;
    return Boolean(FILES[key] && FILES[key].required);
  }

  function fileLabel(key) {
    if (key === "faculty" && state.mode === "housing") return "RA contacts CSV";
  ...
  function clearStoredData() {
    if (!confirm(`Clear locally stored CSV data for ${modeLabel()} and the current report from this browser?`)) return;
    state.files = {};
    state.lastReport = null;
    state.lastInputOptions = null;
    state.sheetsGenerated = false;
  ...
  function buildReport(options) {
    ...
    if (!hasYesterdayHousing && !isHousingMode) warnings.push("Yesterday housing CSV was not uploaded; yesterday evening housing check info will be omitted from sheets.");
    ...
    const students = absentRows.map((attendanceRow) => {
      ...
      if (!id) warnings.push(`Attendance row for ${fullName || "(unknown student)"} has no Pacific ID / participant external ID.`);
      if (id && !activityRow) warnings.push(`No database row matched ${fullName || id}.`);
      ...
      if (!facultyRows.length && (!isHousingMode || hasContactFile)) warnings.push(`No ${isHousingMode ? "RA" : "faculty"} contact row matched ${isHousingMode ? "housing/program" : "program"}: ${contactProgram || "(blank)"}.`);
  ...
  function recordsFromCsv(text, label, warnings) {
    ...
    if (!rows.length || !rows.some((row) => row.some((cell) => cleanCell(cell)))) throw new Error(`${label} CSV is empty.`);

    let headerIndex = rows.findIndex((row) => row.map(normalizeHeader).some((cell) => canon(cell) === "program"));
    if (headerIndex < 0) headerIndex = rows.findIndex((row) => row.some((cell) => cleanCell(cell)));
    if (headerIndex < 0) throw new Error(`${label} CSV is empty.`);

    const hasLiveTitle = rows.slice(0, Math.min(headerIndex + 1, 6)).some((row) => cleanCell(row[0]).includes("Live"));
    if (hasLiveTitle) warnings.push(`${label}: skipped ${headerIndex} Jumbula title/filter row(s) before the header.`);
  ...
  function findRoommates(context) {
    ...
    if (!selfHallKey || !selfRoomBase) return [];
  ...
  function participantExternalId(record, fallbackIndex) {
    ...
    if (fallbackIndex === 8) {
      ...
      if (looksLikePacificId(pickedUpBy)) return pickedUpBy;
    }
  ...
  function firstNonBlank(record, aliases, fallbackIndex) {
    for (const alias of aliases) {
      ...
      if (cleanCell(value)) return value;
    }
