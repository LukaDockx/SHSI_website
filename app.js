...
(() => {
    ...
    function clearStoredData() {
        if (!confirm("Clear all locally stored CSV data and the current report from this browser?"))
            return;
        state.files = {};
        state.lastReport = null;
        state.lastInputOptions = null;
    ...
    function buildReport(options) {
        ...
        const students = absentRows.map((attendanceRow) => {
            ...
            if (!id)
                warnings.push(`Attendance row for ${fullName || "(unknown student)"} has no Participant external ID.`);
            if (id && !activityRow)
    ...
    function parentInfo(row, number) {
        if (!row)
            return { firstName: "", lastName: "", relationship: "", phone: "" };
        if (number === 1) {
            return {
    ...
    function roommateStatusClass(status) {
        ...
        if (normalized === "present" || normalized === "late")
            return "present";
        if (normalized.includes("checked out") || normalized.includes("not checked in") || normalized.includes("not reported") || normalized.includes("needs"))
            return "warn";
        return "";
    }
